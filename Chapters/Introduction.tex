...
Football is a sport of marginal gains. Across the top five European leagues
the average match is decided by a single goal (source), so any repeatable, high-frequency
situation that even slightly shifts the probability of scoring carries clear value. In some cases, even more then one
might expect at first. Corner kicks are one
of the clearest examples. They
occur, on average, 5.5 times per team per match \citep{Casal2015}, they restart
play in the most dangerous third on the pitch, and unlike almost any
phase of open play, they allow prior rehearsal of exact player positions and
...
ShawGopaladesikan2020}. The combination of high frequency and low conversion has long raised the
question of whether corners are genuinely worth investing in, or whether they
amount to a tactical dead end \citep{Power2018}. But not only does this combination still yield 
a substantial number of goals across a season (in the current premier league season, 
corners have directly contributed to approximately 16\,\% of all non-penalty goals), the 
corner goals are also often decisive. When a team does score from a
corner, that goal is the difference between winning or drawing and not in
roughly three out of four cases \citep{Casal2015}. However, research has shown that even
though set pieces acount for 30\% of all goals, teams on average train for only 10 minutes
per week on set pieces \citep{giovanni_vio_2024}, which suggests that there is a significant 
gap between the potential value of set pieces, under which corners, and the resources teams
 allocate to them.
...
set-piece coaches, specialists whose responsibility is the design,
preparation, and analysis of dead-ball situations, are now found at almost
every top-flight club. The career trajectory of Nicolas Jover, who began as a
video analyst at Montpellier, became set-piece coach at Brentford, then
Manchester City, and eventually Arsenal, shows how seriously clubs have come to
take this role. Under Jover, Arsenal led the 2023/24 Premier League in set-piece
goals and have continued to be among the most effective set-piece sides in the
division since.% TODO: verify exact records / season; original draft claimed multiple
% Premier League records — softened until the specific claim can be sourced.
Another set-piece specialist, Gianni Vio, who introduced data-driven corner
routines at Brentford as early as 2015/16, oversaw an 80\,\% improvement in the
club's corner-to-goal conversion rate in a single season. FC Midtjylland,
working under Matthew Benham's analytical approach, scored twenty-five
set-piece goals in one Danish Superliga campaign --- fourteen more than the next
best side.

These are not isolated success stories. They reflect a general recognition that
set pieces, and corners in particular, constitute a market inefficiency:
...
\pounds80 million on a world-class striker. Corner kicks matter, in short,
and the practical question of how to exploit them more effectively, while
preparing against specific opponents who are trying to do the same, is one of
the most consequential open problems in applied football analytics.
...
Despite the growing recognition of corners as a strategic asset, and despite
the rise of dedicated set-piece coaches, the way clubs actually prepare for
corners has not kept pace with the ambition surrounding them. The most acute
expression of this gap is a simple time problem. Although corners and free
kicks together account for roughly a third of goals at elite level
\citep{ShawGopaladesikan2020}, set-piece specialists have observed that
clubs typically dedicate \emph{no more than ten minutes per week of training
time} to set-piece work --- a striking imbalance, given how many goals are
won and lost in those phases over the course of a season. Within that small
window, every minute spent on shaky preparation is a minute lost. The
analytical work that feeds the session is therefore not optional: a coach
who walks onto the pitch with ten minutes and the wrong opponent profile is
in a worse position than one who walks on with no profile at all.

Where preparation is taken seriously, the dominant workflow remains notational
video analysis. If an analyst wants to deliver a usable corner report, they
typically have to watch each of the opponent's recent corners by hand, classify
the delivery, the defensive organisation, and the attacking movement
themselves, and synthesise the result manually. Wright et al.\ (2013) estimated
that performance analysts spend, on average, approximately six hours per match
on opposition preparation; set-piece review absorbs a substantial portion of
that time. The output of this work is genuinely valuable, but it is bought at
a high cost in analyst hours and is therefore rarely as conditional, as
opponent-specific, or as quantitatively grounded as a coach would prefer.

The deeper issue is that even a well-resourced manual analysis is
\emph{fundamentally limited in what it can observe}. A human watching a video
frame can categorise whether a defender appears to be following an attacker
or holding a zone, but cannot precisely quantify the distance between those
two players at each moment leading up to delivery, the velocity with which
the defender is tracking, the degree of synchronisation between defender and
attacker movement, or the spatial coverage gaps that result from the
collective configuration of all ten outfield defenders. These are continuous
spatiotemporal properties of the scene, and they cannot be captured in the
discrete categorical labels that notational analysis produces. As a result,
much of the tactical information embedded in a corner --- information that is
available in the data --- is lost in the process of manual observation.
...
will typically focus on no more than the opponent's five most recent matches,
which yields on the order of thirty corners to work with.% TODO: cite source for the "five recent matches" practice if available;
% otherwise this is grounded in the FC Den Bosch analyst workflow.
Because short corners, anomalous situations, and in-match tactical adjustments
further reduce the homogeneous sample, an analyst may end up with as few as
fifteen to twenty ``standard'' defensive corners from a given opponent to draw
conclusions from.
...
Tracking data offers, in principle, a way out of this bottleneck. When a club
has access to a league-wide tracking database, which is increasingly the case
through commercial data providers, thousands of corners across a full season
can be processed simultaneously. This produces far larger samples per
analytical category, more stable estimates of tactical tendencies, and richer
conditional analyses. It is also a consistent kind of analysis. A human analyst
may code the same corner differently depending on the viewing angle, the
fatigue level of the session, or the ambiguity of a specific player's
movement; an algorithmic pipeline applied to the same tracking data will
always produce the same output. That consistency matters particularly when
the goal is not to describe a single opponent's behaviour, but to build a
general model of which attacking tactics work against which defensive
configurations.

\section{The Promise of Automatic Tactical Retrieval}
\label{sec:prob_automatic}

A second consequence of the scale problem is that, even if an analyst could
review every corner in the league, they would still need a vocabulary in which
to compare them. The two operationally important vocabularies are the
\emph{defensive role} that each individual defender plays at a given corner
(zonal marker, man-marker, short-corner guard, counter-attack defender, and so
on) and the \emph{attacking pattern} that a team executes around the delivery
(the configuration of the runners, the type and arc of the cross, the use of
blockers, and so on). Currently both vocabularies are produced by hand, in
exactly the workflow described above, with all the time, scale, and consistency
problems that entails.

If both vocabularies could be produced \emph{automatically} from tracking and
event data, the value of doing so would be substantial. On the defensive side,
an accurate per-defender role classifier would convert every corner the
opposing team has ever defended in the available dataset, not just the few an
analyst has time to watch, into a structured profile of how that team
typically organises itself. The same opposition profile that today takes hours
to assemble manually would be available immediately and on demand, with full
season-level support rather than five-match anecdote. On the offensive side, an
accurate attacking-pattern classifier would do the same for routines: every
corner taken by the team's own next opponent could be tagged with its routine
type, allowing the coaching staff to see exactly which attacking patterns the
opponent has been faced with most often, and which they have struggled with.

The combination of the two is more valuable still. Once both sides of every
corner are characterised in a shared vocabulary, the conditional question that
motivates set-piece coaching --- \emph{which attacking pattern is most likely
to succeed against this opponent's typical defensive setup?} --- becomes
answerable in seconds rather than hours, and over a full league season rather
than a handful of recent matches. The accuracy of these two classifications is
therefore the load-bearing assumption of everything that follows in this
thesis: every conditional comparison made in SQ3, every opponent profile
generated in SQ5, and every recommendation surfaced in the practical tool
inherits its credibility from the per-defender role labels and per-corner
attacking labels produced in SQ1 and SQ2. For this reason, the empirical work
in this thesis treats SQ1 and SQ2 not as preliminary infrastructure but as
first-class research questions in their own right, with explicit attention to
classification accuracy, validation against manually labelled corners, and an
honest accounting of the irreducible ambiguities (intent-based labels,
incomplete tracking coverage) that bound how accurate either classifier can
be on this dataset.
...
problem, but has done so in a fragmented way. The most important prior work by
Shaw and Gopaladesikan \cite{ShawGopaladesikan2020}, Bauer, Anzer, and
Smith \cite{Bauer2022}, and Wang et al.\ \cite{Wang2024} has
...
individual players. The effectiveness of a corner is determined not only by
which tactical pattern is deployed and against which defensive structure, but
by \emph{who} executes it. Two players in the same tactical role do not
contribute equally. The quality of the corner taker, which can be summarised
through metrics such as average expected assists per delivery, the variance
of the trajectory across deliveries, and the proportion of deliveries that
arrive in the intended zone, governs whether the routine ever has a chance to
work as designed. The aerial ability of the player who is supposed to receive
the cross, characterised by aerial duel win rate, height, jump timing, and
opponent-adjusted measures of aerial dominance, governs whether a delivery
that does arrive in the right zone is converted into a shot. The defensive
side is symmetric: a man-marker who consistently wins headers in his own
penalty area changes the value of the routines that target him, and a
goalkeeper who commands a wide zone changes the value of every floated
delivery into the six-yard box.

These individual contributions are not separable from the tactical analysis.
A routine that performs well on average may perform very differently when
executed by a technically limited taker, or when the primary aerial target is
matched against a dominant defender, or when a low-ranking goalkeeper
behind the zonal block changes the cost--benefit of attacking the near post.
Yet the existing quantitative literature on corners treats players as
interchangeable within their tactical roles and does not model the
interaction between player quality and tactical setup. Recent work on
opponent-adjusted aerial ability \citep{Kim2024} has shown that even a
single player attribute as familiar as the aerial duel win rate can mislead
substantially when treated as a stand-alone metric. The gap is more important
the closer the analysis gets to a coaching decision: at the population level
the question is \emph{what works on average?}, but at the decision level the
question becomes \emph{what should this team do, with these players, against
this opponent?}, and only the second question is the one practitioners
actually face. Closing this gap requires that player quality enter the
effectiveness analysis as an explicit covariate rather than a residual, and
this thesis therefore treats individual quality as a first-class analytical
dimension in SQ4 rather than as a control to be partialled out.
...
%  Maybe add some sort of visualisation of the workflow here? 
...
\subsubsection{Corner Conversion}

Casal et al. \cite{Casal2015} provide the most frequently cited
...
reported a direct goal-per-corner rate of 2.2\,\%. Their multivariate analysis
...
Fern\'andez-Herm\'ogenes, Camerino, and Hileno \cite{FernandezH2021},
working on 2{,}029 corners from the Spanish La Liga and Segunda Divisi\'on,
found that only 29.1\,\% of corners generated a shot attempt which is slighly above 
the earlier range of 17--24\,\% shot-per-corner rates reported across
multiple studies and competitions \citep{Borras2005, SainzdeBaranda2012,
SanchezFlores2012}. A more recent meta-analysis of 21 studies
 on corner-kick performance \cite{JCSS2025} synthesised these findings and concluded
  that the average goal-per-corner rate across elite competitions is approximately 
  2.5\,\%, that the average shot-per-corner rate is approximately 20\,\%, and that 
  the number of attackers in the box is the most consistent predictor of corner success.

\subsubsection{Delivery Characteristics and Target Zones}

Other studies have focused on delivery
characteristics. Pulling \cite{Pulling2015} and Pulling, Robins, and
Rixon \cite{Pulling2013} examined long corners in the English Premier
...
disproportionately many shot attempts. The meta-analysis \citep{JCSS2025} 
synthesised evidence from 21 studies on inswinging versus outswinging deliveries 
and found that outswing corners generate marginally more final attempts in 
domestic leagues, attributing the effect to the ball moving wowards arriving attackers 
rather than away from them. Stafylidis and colleagues \cite{Stafylidis2022, Stafylidis2023}
...
Liu and colleagues \cite{Liu2025} examined 624 corner-kick goals from the Premier League, La Liga, and the Danish Superliga, 
plus 545 World Cup corners, revealing substantial league-level differences in shot-zone 
distributions and goal rates. They identified home advantage, delivery type, non-grouped 
attacking setups, and non-block defensive formations as factors conducive to scoring. 
Critically, their analysis also exposes a methodological limitation common to goal-focused
 studies by conditioning on goals rather than all corners. These methods make setups that led to 
 goals seem more effective than they really are, while underestimating the value of setups 
 that created high-quality chances that were not converted. This study circumvents this
bias through the Corner Threat Score (CTS), which combines delivery quality (xA) with shot 
quality (xG) across \emph{all} corners, including those without subsequent shots \citep{Lamberts2023}, 
and systematically incorporates delivery height variations into its evaluation framework.
Because delivery height --- whether the ball arrives flat and driven, at medium height, or 
as a high-looping float --- remains comparatively under-researched.

\subsubsection{Context, Match Status, and Short Corners}

Studies have also catalogued how context shapes corner
tactics. Casal, Losada, Maneiro, and Ard\'a \cite{Casal2017} used CHAID
...
L\'opez-Riquelme \cite{DeBaranda2012} documented similar effects in the
2006 World Cup. Strafford, Smith, North, and Stone \cite{Strafford2019},
...
consistency did. This observation motivates characterising teams by their
distribution over routine types rather than by the mere presence of any
given routine. Page and Robins \cite{PageRobins2012} conducted a
...
Short corners are corners in which the ball is played to a nearby teammate rather than
crossed directly. They account for roughly 18\,\% of all corner kicks
\citep{Casal2015}, a proportion slightly lower than the 26\,\% (492 of 1{,}893)
...
\citep{Sarmento2025}; this study develops a detailed pass-count and
...
literature. The first one is zone inconsistency: different studies divide the
penalty area into incompatible spatial grids, making cross-study synthesis
unreliable. The second limitation is outcome operationalisation: corner success is
variously defined as a shot, a shot on target, a goal, or the more recent
expected-goals metric, with no consensus standard. Third, contextual
...
status, and player identity. Notational analysis also operates on discrete,
human-assigned labels and therefore cannot access the continuous spatiotemporal
information such as player velocities, mutual distances, movement correlations
that tracking data make available. These simple manual labels compress rich
spatial configurations into single categorical cells, introducing inter-observer
variability and losing the very information that distinguishes a well-executed
zonal block from a poorly coordinated one.
...
work focuses on the systems teams use to organise themselves at corners.
...
zonal marking and man-to-man marking. In a zonal system, each
...
elite football: the hybrid system, which assigns some defenders to zones
...
classified as predominantly zonal when four or more players are assigned
zonal responsibilities, as predominantly man when more than 70\,\% of
...
\cite{PullingNewton2017} showed that near-post guard systems --- in which
...
\cite{Pulling2013} documented substantial cross-club variation in how
...
coverage. Borr\'as and Sainz de Baranda \cite{Borras2005} and
S\'anchez-Flores \cite{SanchezFlores2012} reached analogous conclusions
...
requires defenders to attack the ball rather than track a moving player;
...
reactive by construction: a man-marker who watches his attacker rather
...
specific attacker). MBP School \cite{MBP2025} describes a typical hybrid
...
\cite{Bauer2022} and is reflected in the four-role classification
...
\cite{BeareStone2019}. This static/dynamic dimension is directly reflected
...
Bialkowski and colleagues \cite{Bialkowski2014, Bialkowski2016}
...
Memmert \cite{ReinMemmert2016} established the theoretical framework for
...
constructs. Goes and colleagues \cite{Goes2019, Goes2021} subsequently
...
Power, Hobbs, Ruiz, Wei, and Lucey \cite{Power2018} introduced the first
...
\cite{ShawGopaladesikan2020} is the most methodologically influential
...
Bauer, Anzer, and Smith \cite{Bauer2022} extended the defensive
...
Wang and colleagues \cite{Wang2024} published \emph{TacticAI} in
...
Forte and colleagues \cite{Forte2019}, in an observational analysis of
...
Kim, Lee, and Park \cite{Kim2024}, addressing precisely this confound,
...
attributes. Casal and colleagues \cite{Casal2015, Casal2017}, Pulling and
Newton \cite{PullingNewton2017}, and most notational studies treat players
as interchangeable within their tactical roles. Shaw and Gopaladesikan
\cite{ShawGopaladesikan2020} and Bauer, Anzer, and Smith
\cite{Bauer2022} do not incorporate individual attributes in their
classification models. Wang et al.\ \cite{Wang2024} use position and
